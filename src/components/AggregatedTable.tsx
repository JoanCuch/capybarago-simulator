...
export function AggregatedTable({ aggregatedDays: data }: AggregatedTableProps) {
  return (
    <>
      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '2em' }}>
        <thead>
          <tr>
            ...
            <th style={{ border: '1px solid black', padding: '0.5em' }}>Reached</th>
        ...
        <tbody>
          {data.map((day) => (
            <tr key={day.day}>
              ...
              <td style={{ border: '1px solid black', padding: '0.5em' }}>{day.reachCount}</td>
      ...
      <AggregatedChart aggregatedDays={data} />
    </>
  );
}